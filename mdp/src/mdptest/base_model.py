...
class BaseModel(nn.Module):
    ...
    def forward(self, func_name, **kwargs):
        if func_name == "vision_encoder" :
            output = self.vision_encoder(kwargs["obs_img"], input_goal_mask=kwargs["input_goal_mask"])
        ...
        elif func_name == "pc_net":
           output = self.pc_net(kwargs["xyz_proj"], kwargs["points_proj"], kwargs["xyz_sampled_proj"])
        else:
