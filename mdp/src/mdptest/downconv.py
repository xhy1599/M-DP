...
sys.path.append('/home/dell/visualnav-transformer_104/train/vint_train/models/pointcloud/ops_pytorch/fused_conv_random_k')
sys.path.append('/home/dell/visualnav-transformer_104/train/vint_train/models/pointcloud/ops_pytorch/fused_conv_select_k')
sys.path.append('/home/dell/visualnav-transformer_104/train/vint_train/models/pointcloud/pointnet2')


from fused_conv_random_k import fused_conv_random_k
from fused_conv_select_k import fused_conv_select_k
#from pointnet2_cuda import pointnet2_utils
...
class PointNetSaModule(nn.Module):
    ...
    def __init__(self, batch_size, K_sample, kernel_size, H, W, stride_H, stride_W, distance, in_channels, mlp, bn_decay,
        ...
        # Add final MLPs to get to 16 dimensions
        self.final_mlp1 = Conv2d(self.in_channels, 64, [1,1], stride=[1,1], bn=bn)
        self.final_mlp2 = Conv2d(64, 32, [1,1], stride=[1,1], bn=bn)
        self.final_mlp3 = Conv2d(32, 16, [1,1], stride=[1,1], bn=bn)
    ...
    def forward(self, xyz_proj, points_proj, xyz_sampled_proj):
        ...
        #xyz_proj1=torch.randn(xyz_proj.shape).cuda()
        #xyz_sampled_proj=torch.randn(xyz_sampled_proj.shape).cuda()
        ...
        #print("new_points_group_concat"+str(new_points_group_concat.shape))
        ...
        # Apply final MLPs
        new_points_group_concat = self.final_mlp1(new_points_group_concat)
        new_points_group_concat = self.final_mlp2(new_points_group_concat)
        new_points_group_concat = self.final_mlp3(new_points_group_concat)  # (B, n_sampled, 1, 16)
        
        # Global pooling to get single feature vector
        if self.pooling=='max':
            global_features = torch.max(new_points_group_concat, dim=1)[0]  # (B, 1, 16)
        elif self.pooling=='avg':
            global_features = torch.mean(new_points_group_concat, dim=1)    # (B, 1, 16)
            
        # Reshape to final 1D feature vector
        global_features = global_features.squeeze(1)  # (B, 16)
        #print(f"输出形状: global_features={global_features.shape}")
...
def visualize_pc_project(PC_project, mask=None, title="Cylindrical Projection"):
    ...
    plt.savefig('/home/dell/catkin_ws/src/realsense_pointnet_on_ROS-main/src/huidu.png', bbox_inches='tight', pad_inches=0, dpi=500)  # 保存图像
    plt.close()
    ...
    if mask is not None:
        ...
        plt.ylabel("Elevation (Row Index)")
        #plt.show(block=False)  # 非阻塞模式
        plt.savefig('/home/dell/catkin_ws/src/realsense_pointnet_on_ROS-main/src/mask.png', bbox_inches='tight', pad_inches=0, dpi=500)  # 保存掩码图像
        plt.close()
