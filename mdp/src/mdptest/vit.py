...
class ViT(nn.Module):
    def __init__(
        ...
        self.ViT = MaskedGoalViT(
            ...
            image_size=(self.image_height, self.image_width*self.context_size ),
    ...
    def forward(
        self, obs_img: torch.tensor,  input_goal_mask: torch.tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:

        obs_img_list = list(torch.split(obs_img, 3, dim=1))
        #print(obs_img.shape)#torch.Size([4, 12, 96, 96])
        #print(len(obs_img_list ))#4
        obsgoal_img_list = obs_img_list 
       #  obsgoal_img_list = obs_img_list + [goal_img]
        #print(len(obsgoal_img_list ))#5
        x = torch.cat(obsgoal_img_list, dim=-1)
        #print(x.shape)#torch.Size([4, 3, 96, 480])
        ...
        assert x.shape[3] == self.image_width*self.context_size , f"input image width is not {self.image_width}*(context_size )"
       
        final_repr = self.ViT(x)
      #  print(final_repr.shape)#torch.Size([4, 256])
        return final_repr
...
class MaskedGoalViT(nn.Module):
    def __init__(self, *, context_size, image_size, patch_size, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64):
        ...
        assert self.w % (context_size) == 0, "context_size must be a factor of numbers of patches in width"
        self.goal_mask[:, -self.w//(context_size + 1):] = 0
